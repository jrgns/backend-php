...
<form method="post" action="?q=<?php echo $action_url ?>" enctype="multipart/form-data">
	<div id="obj_name_container">
		...
		<input id="obj_name" name="obj[name]" type="text" class="text" value="<?php echo plain($values['name']) ?>">
	</div>
	...
	<div id="obj_markdown_container">
		...
		<textarea id="obj_markdown" name="obj[markdown]" class="textarea"><?php echo $values['markdown'] ?></textarea>
	</div>
	<div id="obj_body_container">
		<label id="obj_body_label" for="obj_body">Body</label><br>
		<textarea id="obj_body" name="obj[body]" class="textarea"><?php echo $values['body'] ?></textarea>
	</div>
	<!-- TODO Implement this
